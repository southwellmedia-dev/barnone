...
export const QuotePage: React.FC = () => {
  ...
  const services = [
    { id: "epoxy", label: "Epoxy Flooring", icon: Sparkles, desc: "Garage, showroom, industrial" },
    { id: "polishing", label: "Concrete Polishing", icon: Shield, desc: "High-gloss, matte, or satin" },
    { id: "staining", label: "Staining & Sealing", icon: Palette, desc: "Decorative concrete finishes" },
    { id: "repair", label: "Crack Repair", icon: Wrench, desc: "Structural & cosmetic repairs" },
  ];

  const projectTypes = [
    { id: "residential", label: "Residential", icon: Home, desc: "Garage, basement, patio" },
    { id: "commercial", label: "Commercial", icon: Building2, desc: "Office, retail, warehouse" },
  ];
    ...
    <div className="flex items-center gap-1 sm:gap-2">
      {[1, 2, 3, 4].map((s) => (
        <div key={s} className="flex items-center">
          ...
          >
            <span className="skew-x-6">{s < step ? <Check className="w-3 h-3 sm:w-4 sm:h-4" /> : s}</span>
          </div>
          {s < 4 && (
            <div
      ...
      <div className="grid grid-cols-2 gap-3 sm:gap-4 mt-6 sm:mt-8">
        {services.map((service) => {
          ...
          return (
            <button
              key={service.id}
              onClick={() => setFormData({ ...formData, service: service.id as ServiceType })}
              ...
              <div className="flex flex-col sm:flex-row sm:items-start gap-2 sm:gap-4">
                <div
                  className={`w-10 h-10 sm:w-12 sm:h-12 flex items-center justify-center transform -skew-x-6 transition-colors flex-shrink-0 ${
                    isSelected ? "bg-white/20" : "bg-brand-blue/10 group-hover:bg-brand-blue/20"
                  }`}
                >
                  <Icon className={`w-5 h-5 sm:w-6 sm:h-6 transform skew-x-6 ${isSelected ? "text-white" : "text-brand-blue"}`} />
                </div>
                <div className="min-w-0">
                  <h3 className={`font-heading text-sm sm:text-xl italic uppercase leading-tight ${isSelected ? "text-white" : "text-brand-black"}`}>
                    {service.label}
                  </h3>
                  <p className={`font-tech text-xs sm:text-sm mt-1 hidden sm:block ${isSelected ? "text-white/70" : "text-brand-black/50"}`}>
                    {service.desc}
                  </p>
                </div>
              </div>
      ...
      <div className="grid grid-cols-2 gap-3 sm:gap-4 mt-6 sm:mt-8">
        {projectTypes.map((type) => {
          ...
          return (
            <button
              key={type.id}
              onClick={() => setFormData({ ...formData, projectType: type.id as ProjectType })}
            ...
            >
              ...
              >
                <Icon className={`w-6 h-6 sm:w-8 sm:h-8 transform skew-x-6 ${isSelected ? "text-white" : "text-brand-blue"}`} />
              </div>
              <h3 className={`font-heading text-lg sm:text-xl italic uppercase ${isSelected ? "text-white" : "text-brand-black"}`}>
                {type.label}
              </h3>
              <p className={`font-tech text-xs mt-1 sm:mt-2 hidden sm:block ${isSelected ? "text-white/70" : "text-brand-black/50"}`}>
                {type.desc}
              </p>
              {isSelected && (
                <div className="absolute top-2 right-2 sm:top-3 sm:right-3">
  ...
  const renderStep3 = () => (
    <div className="space-y-4 sm:space-y-6 animate-fadeIn">
      ...
      <div className="space-y-4 sm:space-y-6 mt-4 sm:mt-8">
        ...
        <div>
          <label className="block font-tech text-xs font-bold text-brand-black uppercase tracking-widest mb-2 sm:mb-3">
            Additional Details <span className="text-brand-black/40">(Optional)</span>
          </label>
          <textarea
    ...
    <div className="text-center space-y-6 sm:space-y-8 animate-fadeIn">
      ...
      <div>
        ...
        <p className="font-tech text-brand-black/60 mt-3 sm:mt-4 max-w-md mx-auto text-sm sm:text-base">
          Thanks, {formData.name}! We'll review your project details and get back to you within 24 hours with a detailed quote.
        </p>
      </div>
                ...
                <div className="space-y-4 pt-4">
                  {[
                    ...
                    <div key={i} className="flex items-center gap-3">
                      ...
                      <span className="font-tech text-brand-black/80">{item}</span>
                    </div>
                  ))}
                </div>

                {/* Trust Badge */}
                <div className="flex items-center gap-4 pt-8 border-t border-brand-black/10">
                  <div className="w-14 h-14 bg-brand-red flex items-center justify-center transform -skew-x-6">
                    <Award className="w-7 h-7 text-white transform skew-x-6" />
                  </div>
                  <div>
                    <span className="font-heading text-xl italic text-brand-black block">Veteran Owned</span>
                    <span className="font-tech text-brand-black/50 text-xs uppercase tracking-widest">& Operated</span>
                  </div>
                </div>
              </div>
            </div>
